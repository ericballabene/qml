...
def build_qnndnn_model_with_reuploading_multiqubit_correlators_pqc(num_layers=3):
...
def train_qnndnn_model_reuploading_with_multiqubit_correlators(df, weights_path):
    """Train QNN model on given data."""
    ...
    model = build_qnndnn_model_with_reuploading_multiqubit_correlators_pqc()
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=1e-3),
        loss=tf.keras.losses.BinaryCrossentropy(),#'sparse_categorical_crossentropy',
        metrics=['accuracy']
