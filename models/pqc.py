...
def create_reuploading_pqc_with_multiqubit_correlators(num_layers=3):
    ...
    for layer in range(num_layers):
        ...
        for i in range(NUM_QUBITS - 1):
            circuit.append(cirq.CNOT(qubits[i], qubits[i + 1]))
    readout_operators = [cirq.Z(qubits[0]) * cirq.Z(qubits[1]), cirq.Z(qubits[1]) * cirq.Z(qubits[2]), cirq.Z(qubits[2]) * cirq.Z(qubits[3]), cirq.Z(qubits[3]) * cirq.Z(qubits[4]), cirq.Z(qubits[4])]
    return circuit, data_symbols, theta_symbols, readout_operators
